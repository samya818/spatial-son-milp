...
class SONSimulator:
    def __init__(self, policy='dynamic', model_name='xgb_q80', threshold=1.0):
        self.policy = policy
    ...
    def run(self):
        ...
        for slot in tqdm(slots, desc=f"Simulation {self.policy} (T={self.threshold})"):
            ...
            for i, row in enumerate(slot_df.iter_rows(named=True)):
                ...
                if ant_id:
                    antenna_preds[ant_id] = antenna_preds.get(ant_id, 0.0) + float(preds_1h[i])
            
            ant_stats = {a: {'V_a': v, 'C_a': self.topology['antennas'][a]['capacity_mo']} for a, v in antenna_preds.items()}
            
            if self.policy == 'dynamic':
                congested = [a for a, s in ant_stats.items() if s['V_a'] > s['C_a'] * self.threshold]
                if congested:
            ...
            for m_id, current_delta in self.current_offsets.items():
                m_fracs = self.fractions_df.filter((pl.col('master_id') == m_id) & (pl.col('delta_dB') == current_delta))
                cells_in_ant = self.coverage_1024[m_id]
...
if __name__ == "__main__":
    # Test avec Q80 et seuil 1.0 (hard capacity)
    print("RUNNING BENCHMARK Q80 / THRESHOLD 1.0")
    sim_static = SONSimulator(policy='static', model_name='xgb_q80', threshold=1.0)
    res_static = sim_static.run()
    
    sim_dynamic = SONSimulator(policy='dynamic', model_name='xgb_q80', threshold=1.0)
    res_dynamic = sim_dynamic.run()
    
    print("\n" + "="*40)
    print("RÉSULTATS BLOC 1024 CELLULES - Q80 / HARD CAP")
    print("="*40)
